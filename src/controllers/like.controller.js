...
const checkLikeforVideo = asyncHandler(async (req, res) => {
  const { videoId } = req.params;
  if (!isValidObjectId(videoId)) {
    throw new ApiError(400, "Invalid video id");
  }
  ...
  const like = await Like.findOne({ video: videoId, likedBy: user._id });
  if (like) {
    return res.status(200).json(new ApiResponse(200, true, "Video liked"));
  }
  return res.status(200).json(new ApiResponse(200, false, "Video not liked"));
});
export {
  toggleCommentLike,
  toggleTweetLike,
